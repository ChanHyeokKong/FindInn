			...
			hotels.forEach(hotel => {
				...
				geocoder.addressSearch(address, function(result, status) {
					if (status === kakao.maps.services.Status.OK) {
						...
						let infoCard = null; // 이 변수가 이 위치에 선언되어 있는지 확인해주세요.

						content.addEventListener('mouseenter', () => {
							// 이미 infoCard가 있다면 중복 생성하지 않음
							if (infoCard) {
								return;
							}

							// CustomOverlay의 z-index를 높여서 다른 마커들보다 앞에 오게 함
							customOverlay.setZIndex(2000);

							// infoCard를 생성하고 CSS 클래스를 적용
							infoCard = document.createElement('div');
							infoCard.className = 'hotel-card map-hotel-card';
							...
							// 이미지 경로 처리
							const imgUrl = hotel.hotelImage ? `/uploads/hotels/${hotel.hotelImage}` : '';

							infoCard.innerHTML = `
						                                <div class="d-flex" style="flex-direction: row; align-items: flex-start; padding: 10px;">
						                                    <div class="hotel-image me-3" style="width: 120px; height: 120px; flex-shrink: 0;">
						                                        <img src="${imgUrl}" alt="호텔 이미지" loading="lazy" style="width: 100%; height: 100%; object-fit: cover; border-radius: 8px;" />
						                                    </div>
						                                    <div class="hotel-info" style="flex-grow: 1; min-width: 0;">
						                                        <h3 class="hotel-title" style="font-size: 1rem; font-weight: bold; margin-bottom: 5px; line-height: 1.3; overflow-wrap: break-word; white-space: normal;">${hotel.hotelName}</h3>
						                                        <div class="hotel-address" style="font-size: 0.8rem; color: #6c757d; margin-bottom: 5px; line-height: 1.4; overflow-wrap: break-word; white-space: normal;">
						                                            <i class="fas fa-map-marker-alt text-primary me-2"></i>
						                                            <span>${hotel.hotelAddress}</span>
						                                        </div>
						                                        <div class="hotel-rating mb-2" style="font-size: 0.8rem;">
						                                            <div class="rating-stars me-2" style="display: inline-block;">
						                                                ${starsHtml}
						                                            </div>
						                                            ${ratingText}
						                                        </div>
						                                        <div class="price-tag mt-2" style="text-align: right;">
						                                            <strong style="font-size: 1.1rem;">₩${Number(hotel.priceRange).toLocaleString()}</strong>
						                                            <small class="d-block opacity-75" style="font-size: 0.7rem;">(1박 기준)</small>
						                                        </div>
						                                    </div>
						                                </div>
						                            `;
							...
							// ⭐ ⭐ ⭐ 추가된 부분 ⭐ ⭐ ⭐
							// infoCard에 클릭 이벤트 리스너 추가 (클릭 시 상세페이지 이동)
